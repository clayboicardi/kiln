...
3. **Execute the 2b-A UI: A4 → A5 → A6** from the plan `docs/superpowers/plans/2026-06-19-phase-2b-a-spec-sheet.md`, subagent-driven (opus subagents, per Clay). SDD ledger: `/tmp/kiln-sdd/progress.md` (regenerate if /tmp was cleared — the engram resume-point memory has the full state).
...
- **A5 — `SpecSheetScreen` reads real data + nav threading.** `SpecSheetScreen(trackId, statsSource)` collects `LibraryStatsSource.specSheetEntry()` + `aggregateStats()`. Thread `LibraryStatsSource` through `NowPlayingTab` → `NowPlayingHomeScreen` → `SpecSheetScreen`. **Discovery step:** `grep -rn "NowPlayingTab(" app-android app-desktop ui` to find construction sites in both apps; expose `abstract val libraryStats: LibraryStatsSource` on both DI graphs (same `LocalLibrarySource` instance). NOTE: `:ui:components` already deps `:data:library`; it consumes a `Flow` so it does NOT need `libs.bundles.sqldelight.common`.
