...
- **Android** (`AndroidAppGraph.sqlDriver()` `Callback.onOpen`): add `PRAGMA busy_timeout = 5000` next to the existing `PRAGMA foreign_keys = ON` (per-connection, so it must run on every open). WAL is the framework/Requery default for a writable `SupportSQLiteOpenHelper`; **verify on-device** (`PRAGMA journal_mode`) and set it explicitly if the bundled Requery build does not default to WAL.
...
- **`AndroidFormatFactBackfill`:** gains a `DatabaseWriter`. Native reads (`MediaExtractor` / `MediaMetadataRetriever`) stay off-writer (on `ioDispatcher`); each page's `db.transaction` write goes through `writer.write { }`. The Android scanner calls `backfill.runOnce()` **after** (sequentially with) its own `writer.write` block — not nested (it is `suspend`).
