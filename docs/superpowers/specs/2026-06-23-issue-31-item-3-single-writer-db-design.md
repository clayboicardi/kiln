...
- **Android** (`AndroidAppGraph.sqlDriver()` `Callback`): enable WAL the **framework-approved** way — `db.enableWriteAheadLogging()` in `onConfigure` (NOT a raw `PRAGMA journal_mode=WAL` via `execSQL`: that does not configure the framework's connection pool, and `execSQL` on a row-returning PRAGMA can throw). **Move FK enforcement to `onConfigure` via `setForeignKeyConstraintsEnabled(true)`** — WAL opens a connection pool, so a raw `PRAGMA foreign_keys` in `onOpen` only covers the primary connection (writes on other pooled connections would skip FK checks); the framework setter applies pool-wide (codex round-2, PR #34). This also refines the CLAUDE.md "onOpen runs per connection" note — that held in the pre-WAL single-connection regime; with the pool it does not. Keep the per-connection `PRAGMA busy_timeout = 5000` in `onOpen` (best-effort; on Android WAL + the pool are the primary concurrency mechanism). (gemini G1 / codex C3, PR #34.) Verify on-device with `PRAGMA journal_mode` → `wal`.
